...
(function($) {
    ...
    /** 年月絞り込み条件の、表示開始年 */
    var DISP_START_MONTH_NUM = 2010;    // 2010年～2019年(来年？)
     ...
     * 引数の年月部分を選択状態にします。
     *
     * DISP_START_MONTH_NUM(2010)年～来年(今が2018年なら2019年)までの選択肢を作成します
     */
    var nowDate = moment();
    var makeSelectYearMonth = function(seleYear, seleMonth, nameYear, nameMonth){
        return makeSelectYearMonth_fromToYear(DISP_START_MONTH_NUM, nowDate.year() + 1, seleYear, seleMonth, nameYear, nameMonth);
    }
    ...
    var makeWhereString = function(paramName, connector, operator, list) {
        ...
        let st = st1 + list2.join( ' ' + connector + ' ' + st1);
        return st;
    };
    ...
    const addIntegrateByCharge = function(source, dist, chargeTemplate, jobTemplate) {
        ...
        let chargeCode = source['担当者コード']['value'];
        if (!(chargeCode in dist['担当者毎集計'])) {
            dist['担当者毎集計'][chargeCode] = $.extend(true, {}, chargeTemplate);
            dist['担当者毎集計'][chargeCode]['担当者毎集計担当者区分'] = (isMainStaff(chargeCode, dist['顧客コード']) ? '担当' : '非');
            dist['担当者毎集計'][chargeCode]['担当者毎集計担当者名称'] = source['担当者名']['value'];
    ...
    var integrateTotal = function(list, dest){
        ...
        let breakKey;
        for (let ix = 0; ix < list.length; ix++) {
            let key = (list[ix]['顧客コード'] || list[ix]['担当者コード']);
    ...
    var getPeriodFromTo = function () {
        ...
        if (toNumber(fromYear) > toNumber(toYear)) { return ;}
        if (toNumber(fromYear) === toNumber(toYear) && toNumber(fromMonth) > toNumber(toMonth)) { return ;}
        let period = {};
        period.from = {};
        period.to = {};
        ...
        return kintoneUtility.rest.getAllRecordsByQuery({
                    ...
                    for (let ix = 0; ix < records.length; ix++) {
                        rec[records[ix]['担当者コード'].value] = {
                            ...
                            hourReward: records[ix]['時給'].value, // 時給
    ...
    var getCustomerList = function (period, whereOption, period2) {
        let isAllList = (period === void 0);
        let querySt = ' order by 顧客コード asc ';
        if (!isAllList) {
            ...
            if (whereOption) {
                ...
                if (whereOption.keyword && whereOption.keyword.customer){
                    if (whereOption.keyword.customer.length > 0) {
                        querySt = ' and (' +
                            makeWhereString('顧客コード', 'or', '=', whereOption.keyword.customer) + ') ' +
                            querySt;
                    } else {
                        // 1件も対象にない場合は、1件も引っかからないようにする。
                        querySt = ' and (顧客コード = "") ' +
                ...
                if (whereOption.mainChargeCustomers) {
                    if (whereOption.mainChargeCustomers.length > 0) {
                        querySt = ' and (' +
                            makeWhereString('顧客コード', 'or', '=', whereOption.mainChargeCustomers) + ') ' +
                            querySt;
                    } else {
                        // 1件も対象にない場合は、1件も引っかからないようにする。
                        querySt = ' and (顧客コード = "") ' +
    ...
    var getDailyReportAppData = function (query, option) {
        return kintoneUtility.rest.getAllRecordsByQuery({
                    ...
                    for (let ix = 0; ix < resp.records.length; ix++) {
                        ...
                        one['年月'] = { value: makeYearMonthSt(one['日付'].value) };
                        let chargeCode = resp.records[ix]['担当者コード'].value;
                        for(let iy = 0; iy < resp.records[ix]['Table'].value.length; iy++) {
                            let customerCode = resp.records[ix]['Table'].value[iy].value['顧客コード'].value;
                            let categoryCode = resp.records[ix]['Table'].value[iy].value['区分コード'].value;
                            // 絞り込み(顧客未選択の工数を除く)
                            if (option.expectNoCustomer) {
                                // 顧客コードが空ではなく、存在する顧客コード→利用／そうでなければ不使用
                                // 存在しない顧客コードの指定された日報登録行は弾く(期間内の有効な契約の顧客コードでの判定ではない)
...
//                                } else {
                            ...
                            if (option.mainChargeCustomers){
                                if (option.mainChargeCustomers.indexOf(customerCode) < 0) {
                                    // 指定の顧客コード配列に存在しないものは排除
                                    continue;
                                }
                            }
                            // キーワードの絞り込み
                            if (option.keyword && option.keyword.customer) {
                                // 顧客コードの絞り込みのみ対応(担当者コードの絞込みはクエリで対応)
                            ...
                            let workMin = getMinutes(tableOne['計算_作業時間'].value);
    ...
    /**
     * 指定の期間中に契約中であり、指定の担当者コードが主担当である顧客コードのリストを返します。
     * 期間の指定がない場合は例外を発生します
     *
     * @param chargeCode 担当者コード
    ...
    var getMainChargeCustomerCodeList = function(chargeCode, period) {
        ...
        for (let ix = 0; ix < list.length; ix++) {
            if (list[ix]['担当者コード'].value === chargeCode) {
                if ( havingContract(myVal.ALL_LIST_CUSTOMER[list[ix]['顧客コード'].value], period)) {
                    // 契約期間チェックがある場合はこちら
                    dest.push(list[ix]['顧客コード'].value);
                }
            }
        }
        return dest;
    ...
    /**
     * 指定の文字列を含む(部分一致)顧客コードのリスト(配列)を返します。
     * 顧客名か顧客コードかのどちらかに指定の文字列が含まれているものを返します
     * @return 顧客コードの配列(空の配列の場合あり)
     */
    var getCustomerCodeListByBroadKeyword = function(keyword) {
        let dest = [];
        let list = myVal.ALL_SRC_LIST_CUSTOMER;
        for (let ix = 0; ix < list.length; ix++) {
            if (list[ix]['顧客名'].value.indexOf(keyword) >= 0 || list[ix]['顧客コード'].value.indexOf(keyword) >= 0) {
                dest.push(list[ix]['顧客コード'].value);
            }
        }
        return dest;
     ...
     * パラメータに顧客コードが存在する場合、その顧客コードのみの月額報酬を返します。
     * リストが存在しない場合は、''を返します。
     *
     * @param chargeCode 担当者コード
     * @param period 指定されている場合、契約が有効な場合のみ値を返します。
     *  periodの例) {fromDate: '2017-01-01', toDate: '2018-04-30'}
     * @param customerCode 顧客コード(指定の担当者の月額報酬のうち、指定の顧客コードの月額報酬のみを返します)
     */
    var getMainChargeCustomerReward = function (chargeCode, period, customerCode) {
        ...
        for (let ix = 0; ix < customerCodeList.length; ix++) {
            let reward = getCustomerReward(customerCodeList[ix]);
    ...
    /**
     * 顧客コードから顧客の月額報酬額を返します。
     * リストが存在しない場合は、''を返します。
     * @param period 指定されている場合、契約が有効な場合のみ値を返します。
     *  periodの例) {fromDate: '2017-01-01', toDate: '2018-04-30'}
     */
    var getCustomerReward = function (customerCode, period) {
    ...
    /**
     * 担当者コード、顧客コードを元に、指定顧客の主担当者かどうかを判定します。
     *
     * @param chargeCode 担当者コード
...
//        console.log('金額増減render: value: '+ value + ' text:' + td.innerText);
    ...
    var makeByChargeList = function(sourceList, dataListDest, period){
        ...
        // 存在しない担当者用の行
        let noCodeOne = {担当者名: DISP_VALUE_NOT_EXIST_CHARGE_NAME, no_code: true,
        ...
        for (let ix = 0, iy = 0; ix < allList.length ; ix++) {
            ...
            if (dataListDest.length === ix) {
                // 現行分の1行分データを作成
                let one = { '担当者コード': code, '担当者名': getChargeName(code),
                            報酬: getMainChargeCustomerReward(code, period), 工数_分: '',
                            工数: '', 人件費: '',           // アプリデータ
                            年月: '',   //debug用
                            損益: '', 時間あたり: '', 利益率: ''};  // 計算結果格納用
                dataListDest.push(one);
            }
            // ソースデータを回す
            for (; iy < sourceList.length ; iy++) {
                // 全行回す
                if ( getChargeName(sourceList[iy]['担当者コード'].value, period) === SYS_VALUE_NOT_EXIST_NAME) {
                    addWorkingHourPersonCost(sourceList[iy], [noCodeOne]);
                } else if (sourceList[iy]['担当者コード'].value !== code) {
    ...
    var makeByCustomerList = function(sourceList, dataListDest, period, option){
        ...
        let noCodeOne = {顧客名: DISP_VALUE_NOT_EXIST_CUSTOMER_NAME, no_code: true};
        for (let ix = 0, iy = 0; ix < allList.length ; ix++) {
            let code = allList[ix]['顧客コード'].value
            if (code == "") {
                console.log("み選択");
            }
            // 当顧客の月額報酬
            ...
            for (; iy < sourceList.length ; iy++) {
                // 全行回す
                if ( getCustomerName(sourceList[iy]['顧客コード'].value) === SYS_VALUE_NOT_EXIST_NAME) {
                    // 顧客コードが存在しない or マスタに存在しないコードは専用のnoCodeOneに加算
                    // (顧客未選択の工数を除く)場合はtotalに加算しない
                    addWorkingHourPersonCost(sourceList[iy], [noCodeOne]);
                } else if (sourceList[iy]['顧客コード'].value !== code) {
                    // 存在するコードだけどallリストの現行と不一致なら、allリストの次の行に
                    break;
                } else {
                    // コードが一致してるなら、加算
     ...
     * 顧客コード未指定/該当コード顧客不在(有効期限外含む)、または該当担当者不在(有効期限外含む)の場合の合計行を追加
     *
     ...
     * 集約は担当者毎/顧客毎のいずれかの集約で、codeName('担当者コード'/'顧客コード')で指定します。
...
/************************************* 生産性/損益分析用メイン処理 ********************************************/
        ...
        let html4 = '<tr><th>オプション</th><td colspan="3">' + 
            '<div class="check-option-choice" id="">' +
                '<span id="op1" class="check-option-choice-row" style="-ms-grid-row:1;">' +
                    '<input type="checkbox" id="ch3"></input><label for="ch3">報酬、' + dispStr + 'がないものを非表示にする</label></span>' +
    ...
    var dispTowTerm = function(period, showType) {
        ...
        let useCodeName = (showType === '顧客別')? '顧客コード' : '担当者コード';
        ...
        // fromの1日からtoの月末の日付まで
        let periodComp = {};
        periodComp.from = {month: period.from.month, year: period.from.year - 1};
        periodComp.to = {month: period.to.month, year: period.to.year - 1};
        periodComp.moment = {from: period.moment.from.clone().add(-1, 'years'), to: period.moment.to.clone().add(-1, 'years')};
        ...
        // }).then(function() { 
            ...
            //////// 取得データの加工 ////////
            let totalRec = {'報酬': '', '人件費': '', '工数': '', '担当者名': '合計', '損益': '', '利益率': ''};
            let totalRecComp = {'報酬': '', '人件費': '', '工数': '', '担当者名': '合計', '損益': '', '利益率': ''};
    ...
    var dispOneTerm = function(period, showType) {
        ...
        getPersonOfChargeList(period, whereOption) .then(function() {
            ...
            if (unFormattedList[unFormattedList.length - 1].no_code) {
                unFormattedList[unFormattedList.length - 1]['顧客コード'] = "";
            ...
            const customSort = function(selectedColumn, sortConfig) {
                dataList.sort(function(a, b) {
                    ...
                    // コードの昇順は全てのケースで行う
                    if (a[(showType === '顧客別' ? '顧客コード' : '担当者コード')] < b[(showType === '顧客別' ? '顧客コード' : '担当者コード')]) return -1;
                    if (a[(showType === '顧客別' ? '顧客コード' : '担当者コード')] > b[(showType === '顧客別' ? '顧客コード' : '担当者コード')]) return 1;
            ...
            if (dispViewId == emxasConf.getConfig('VIEW_PRODUCTIVITY_ANALYSIS')) {
                ...
                $(document).on('click', 'table.htCore span.colHeader', function(e) {
                    ...
                    // 現在降順 and 前回ソートと同じ列をクリック ⇒ 顧客コード／担当者コード順でソート（初期表示状態）
                    } else if (beforeSortConfig === 1 && clickedColumn === sortedColumn) {
                        sortConfig = 0;   // ソート無し
                        clickedColumn = 0; // 顧客コード／担当者コード列をクリックした事にする。
                        e.target.classList.remove("descending");
                        e.target.classList.remove("ascending");
                    } 
    ...
    function makeWhereOption(showType, useCodeName, period){
        ...
        if (showType === '顧客別') {
            ...
            if (val !== 'all') {
                ...
                whereOption.query += ' and 担当者コード = "' + val + '"';
            } else {
        ...
        if (keyword && keyword !== '') {
            ...
            if (showType === '顧客別') {
                ...
                if (cl.length > 0) {
                    whereOption.query += ' and ('+ makeWhereString('担当者コード', 'or', '=', cl) + ')';
