...
(function($) {
    ...
    function createUserGraph(susr) {
		...
		fetchRecords(SALES_APPID, '((案件期間開始 <= ' + maxMonthU + ' and 案件期間開始 >= ' + minMonthU + ') or (案件期間終了 <= ' + maxMonthU + ' and 案件期間終了 >= ' + minMonthU + ')) and 担当者 in ("' + scod + '") order by 顧客コード asc')
		.then(function(user1Canvas) {
		    fetchRecords(SALES_APPID, '((案件期間開始 <= ' + maxMonthU2 + ' and 案件期間開始 >= ' + minMonthU2 + ') or (案件期間終了 <= ' + maxMonthU2 + ' and 案件期間終了 >= ' + minMonthU2 + ')) and 担当者 in ("' + scod + '") order by 顧客コード asc')
