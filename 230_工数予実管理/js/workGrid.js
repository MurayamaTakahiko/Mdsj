...
(function($) {
  ...
  function fetchRecords(appId, usr, opt_offset, opt_limit, opt_records) {
    ...
    if (!usr || usr === "--") {
      var params = {
        app: app_plan,
        query: 'order by 対象月初日 desc limit ' + limit + ' offset ' + offset
      };
    } else {
      var params = {
        app: app_plan,
        query: '検索用担当者 = "' + usr + '" order by 対象月初日 desc limit ' + limit + ' offset ' + offset
      };
    }
  ...
  function makeYojitsuData(records, opt_data, opt_i, user_list) {
    ...
    var params = {
      'app': appId,
      'query': '担当者名 = "' + key1 + '" and 日付 >= "' + key3Start + '" and 日付 <= "' + key3End + '"'
    };
