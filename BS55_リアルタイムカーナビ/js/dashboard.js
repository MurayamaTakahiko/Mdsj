...
(function($) {
    ...
    function createCompareGraph(min, max) {
        ...
        // 今月から一年前までのレコード取得
		fetchRecords(SALES_APPID, '(案件期間開始 <= ' + maxMonth + ' and 案件期間開始 >= ' + minMonth + ') or (案件期間終了 <= ' + maxMonth + ' and 案件期間終了 >= ' + minMonth + ') order by 顧客コード asc')
		.then(function(canvas1Rec) {
			...
			.then(function(canvas1Rec2) {
				...
				for (var v = 0; v < canvas1Rec2.length; v++) {
					...
					if (moment(canvas1Rec2[v].決算開始日.value).isSameOrBefore(moment())) {
				...
				fetchRecords(SALES_APPID, '(案件期間開始 <= ' + maxMonth2 + ' and 案件期間開始 >= ' + minMonth2 + ') or (案件期間終了 <= ' + maxMonth2 + ' and 案件期間終了 >= ' + minMonth2 + ') order by 顧客コード asc')
				.then(function(canvas1Rec2) {
					data = refreshData(data);
					// 前年同月比グラフのデータ作成
          ...
          data[noMonthBefore] = data[oneMonthBefore] + eventuallyPlanThisYearIncome[11];
					...
					for (var l = 0; l < canvas1Rec.length; l++) {
						...
						var user = canvas1Rec[l].担当者.value;
						...
						data: {
							...
							datasets: [
								...
								},
								{
									yAxisID: "y-axis-0",
									type: 'bar',
									label: min ? moment(min).format('YYYY年M月～') + moment(max).format('YYYY年M月売上＋予測') :
									moment().add(-11, "months").format('YYYY年M月～') + moment().format('YYYY年M月売上＋予測'),
						...
						data: {
							...
							datasets: [
								...
								},
								{
									yAxisID: "y-axis-0",
									type: 'bar',
									label: min ? moment(min).format('YYYY年M月～') + moment(max).format('YYYY年M月利益＋予測') :
									moment().add(-11, "months").format('YYYY年M月～') + moment().format('YYYY年M月利益＋予測'),
    ...
    function createProdGraph(sprd) {
        ...
        SALES_APPID = emxasConf.getConfig('APP_SALES_APPID');
		fetchRecords(SALES_APPID, '((案件期間開始 <= ' + maxMonthP + ' and 案件期間開始 >= ' + minMonthP + ') or (案件期間終了 <= ' + maxMonthP + ' and 案件期間終了 >= ' + minMonthP + ')) and 案件タイプ in ("' + sprd + '") order by 顧客コード asc')
		.then(function(prod1Canvas) {
		    fetchRecords(SALES_APPID, '((案件期間開始 <= ' + maxMonthP2 + ' and 案件期間開始 >= ' + minMonthP2 + ') or (案件期間終了 <= ' + maxMonthP2 + ' and 案件期間終了 >= ' + minMonthP2 + ')) and 案件タイプ in ("' + sprd + '") order by 顧客コード asc')
        ...
        // 対計画比の算出
                ...
                yAxisID: "y-axis-0",
								type: 'bar',
								label: minMonthP ? moment(minMonthP).format('YYYY年M月～') + moment(maxMonthP).format('YYYY年M月売上＋予測') :
								moment(minMonthP2).format('YYYY年M月～') + moment(maxMonthP2).format('YYYY年M月売上＋予測'),
				...
				chart4 = new Chart(ctx13, {
					...
					data: {
						...
						datasets: [
							...
							{
								label: minP ? moment(minP).format('YYYY年M月～') + moment(maxP).format('YYYY年M月売上＋予測') :
								moment().add(-11, "months").format('YYYY年M月～') + moment().format('YYYY年M月売上＋予測'),
    ...
    function createUserGraph(susr) {
        ...
        var scod = susr;
        SALES_APPID = emxasConf.getConfig('APP_SALES_APPID');
		fetchRecords(SALES_APPID, '((案件期間開始 <= ' + maxMonthU + ' and 案件期間開始 >= ' + minMonthU + ') or (案件期間終了 <= ' + maxMonthU + ' and 案件期間終了 >= ' + minMonthU + ')) and 業務担当者 in ("' + scod + '") order by 顧客コード asc')
		.then(function(user1Canvas) {
		    fetchRecords(SALES_APPID, '((案件期間開始 <= ' + maxMonthU2 + ' and 案件期間開始 >= ' + minMonthU2 + ') or (案件期間終了 <= ' + maxMonthU2 + ' and 案件期間終了 >= ' + minMonthU2 + ')) and 業務担当者 in ("' + scod + '") order by 顧客コード asc')
				...
				for (var ii = 0; ii < user1Canvas.length; ii++) {
					...
					for (var jj = 0; jj < tableRecordsU.length; jj++) {
						if (moment(tableRecordsU[jj].value['売上月'].value).isSameOrBefore(maxMonthU) && moment(tableRecordsU[jj].value['売上月'].value).isSameOrAfter(minMonthU)) {
						    if (Object.keys(tableRecordsU[jj].value['業務担当者'].value).length !== 0) {
    							if (tableRecordsU[jj].value['業務担当者'].value == scod) {
				...
				for (var kk = 0; kk < user1Canvas.length; kk++) {
					...
					for (var ll = 0; ll < tableRecordsU2.length; ll++) {
						if (moment(tableRecordsU2[ll].value['売上月'].value).isSameOrBefore(maxMonthU) && moment(tableRecordsU2[ll].value['売上月'].value).isSameOrAfter(minMonthU)) {
						    if (Object.keys(tableRecordsU2[ll].value['業務担当者'].value).length !== 0) {
    							if (tableRecordsU2[ll].value['業務担当者'].value == scod) {
				...
				for (var mm = 0; mm < user1Canvas2.length; mm++) {
					...
					for (var nn = 0; nn < tableRecordsU3.length; nn++) {
						if (moment(tableRecordsU3[nn].value['売上月'].value).isSameOrBefore(maxMonthU2) && moment(tableRecordsU3[nn].value['売上月'].value).isSameOrAfter(minMonthU2)) {
						    if (Object.keys(tableRecordsU3[nn].value['業務担当者'].value).length !== 0) {
                                if (tableRecordsU3[nn].value['業務担当者'].value == scod) {
					...
					data: {
						...
						datasets: [
							...
							},
							{
								yAxisID: "y-axis-0",
								type: 'bar',
								label: minMonthU ? moment(minMonthU).format('YYYY年M月～') + moment(maxMonthU).format('YYYY年M月売上＋予測') :
								moment(minMonthU2).format('YYYY年M月～') + moment(maxMonthU2).format('YYYY年M月売上＋予測'),
				...
				chart4 = new Chart(ctx11, {
					...
					data: {
						...
						datasets: [
							...
							{
								label: minU ? moment(minU).format('YYYY年M月～') + moment(maxU).format('YYYY年M月売上＋予測') :
								moment().add(-11, "months").format('YYYY年M月～') + moment().format('YYYY年M月売上＋予測'),
