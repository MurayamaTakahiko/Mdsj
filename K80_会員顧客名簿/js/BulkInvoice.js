              ...
              subrec[j]['value']['契約番号'].value != "" ){
                ...
                if(targetflg){
                 var max=0;
                  ...
                  if(subrec[j]['value']['オプション利用開始日'].value>=stateldt){
                    stateldt=moment(subrec[j]['value']['オプション利用開始日'].value).startOf('month').format();
                    max=stateldt.diff(prevenddt, 'months')+1   // ;
                  }
